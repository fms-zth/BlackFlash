...
uint32_t swizzle_addr(const __nv_bfloat16* base,int row ,int col )
{
    int chunk=col/8;
    int in_chunk=col%8;
...
__global__ void flash_attn_naive_kernel(
    ...
    // offset = ((b * H + h) * N) * d
    const int bh_offset = (b * gridDim.y + h) * N;  //注意这个是行单位！少了d列的！后面会乘，这里的索引不要想象成三维坐标，这里是一维的，根据B,H,N,来思考索引地址
    //还是那句话，griddim代表什么要看下面dim3代码传的什么  dim3 grid(num_q_blocks, H, B);。所以这里的griddim.y代表的是H的数量
    ...
    __nv_bfloat16* sQ = reinterpret_cast<__nv_bfloat16*>(smem);//这里要把内存类型转换成bf16，前面就要加reinterpret_cast
    //跳过sq的内存，所以用br
        ...
        if (tid%32==0)
        {
            for (int j=0;j<num_kv_blocks;j++)
            {
                ...
                if (j>=2)
                {
                    ...
                    if (round==0) phase_empty0++;
                    else phase_empty1++;//加一重新到达当前的状态
                }
...
//                    half:0  1
    ...
    for (int j = 0; j < num_kv_blocks; j++) {
        ...
        :: "r"((uint32_t)__cvta_generic_to_shared(mbar_full)),
           "r"(phase_cur));
           if (round == 0) phase_full0++; else phase_full1++;


            #pragma unroll
            for (int nc = 0; nc < 4; nc++) {
                RC[nc][0] = 0.0f; RC[nc][1] = 0.0f;
                RC[nc][2] = 0.0f; RC[nc][3] = 0.0f;
            }
...
//搬运完了，重新计算O=P*V；
        ...
        for (int nc=0;nc<4;nc++)
        {
            for (int kc=0;kc<4;kc++)
            {
                ...
                HMMA16816(
                    ROC[nc][0],ROC[nc][1],ROC[nc][2],ROC[nc][3],
                    RPA[0],RPA[1],RPA[2],RPA[3],
                    RVB[0],RVB[1],
                    ROC[nc][0],ROC[nc][1],ROC[nc][2],ROC[nc][3]
                );
            }
...
void flash_attn_naive_launch(
    ...
    size_t smem_size = (Br * d_model + Bc * d_model*2 + 2*Bc * d_model)
                       * sizeof(__nv_bfloat16)+5*sizeof(uint64_t)+4 * Br * sizeof(float);
    ...
    cuuint32_t q_element_stride[2]={1,1,};

    cuTensorMapEncodeTiled(
        &tma_desc_q,
        CU_TENSOR_MAP_DATA_TYPE_BFLOAT16,
        2, (void*)Q,
        q_g_dim, q_g_stride,q_b_dim,q_element_stride,
    ...
    cuTensorMapEncodeTiled(
        ...
        CU_TENSOR_MAP_DATA_TYPE_BFLOAT16,
        2, (void*)K,
        q_g_dim, q_g_stride, K_boxDim,q_element_stride,
        CU_TENSOR_MAP_INTERLEAVE_NONE,
        CU_TENSOR_MAP_SWIZZLE_128B,
        CU_TENSOR_MAP_L2_PROMOTION_NONE,
        CU_TENSOR_MAP_FLOAT_OOB_FILL_NAN_REQUEST_ZERO_FMA
    ...
    if (smem_size > 48 * 1024) {
        CUDA_CHECK(cudaFuncSetAttribute(
            flash_attn_naive_kernel,
            cudaFuncAttributeMaxDynamicSharedMemorySize,
            smem_size
        ));
    }

    flash_attn_naive_kernel<<<grid, block, smem_size, stream>>>(
        tma_desc_q, tma_desc_k, tma_desc_v, O, l, m, N, scale
    );
    CUDA_KERNEL_CHECK();
}
