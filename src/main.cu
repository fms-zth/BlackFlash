...
static void compare(const char* label, 
                    const float* gpu, const float* ref, 
    ...
    if (max_err >= tol) {
        ...
        printf("  Ref: ");
        for (int j = 0; j < dbg_cols; j++) printf("%8.4f ", ref[max_r * cols + j]);
        printf("\n  GPU: ");
        for (int j = 0; j < dbg_cols; j++) printf("%8.4f ", gpu[max_r * cols + j]);
        printf("\n");
    }
}
...
int main(int argc, char** argv) {
    ...
    int B = 0, H = 0, N = 0, d = 0;
    float scale = 0.0f;
    if (!read_meta(ref_dir, B, H, N, d, scale)) return 1;
    ...
    if (!read_bin(ref_dir, "q.bin", h_Q.data(), qkv_elems * sizeof(__nv_bfloat16))) return 1;
    if (!read_bin(ref_dir, "k.bin", h_K.data(), qkv_elems * sizeof(__nv_bfloat16))) return 1;
    if (!read_bin(ref_dir, "v.bin", h_V.data(), qkv_elems * sizeof(__nv_bfloat16))) return 1;
    if (!read_bin(ref_dir, "o_ref.bin", h_O_ref_bf16.data(), qkv_elems * sizeof(__nv_bfloat16))) return 1;
    if (!read_bin(ref_dir, "s_ref.bin", h_S_ref.data(), s_elems * sizeof(float))) return 1;
...
for (int i = 0; i < 8; i++) printf("  Q[%d] = %.6f\n", i, tmp[i]);
...
// ============================================================
{
    ...
    printf("(Compares GPU output as S against S_ref, first %d columns)\n", d);
    {
        ...
        compare("S (debug)", s_gpu_trimmed.data(), s_ref_trimmed.data(),
                N, compare_cols, N * compare_cols, 0.05f);
    }

    // --- (b) O 对比 (完整 FA2 模式) ---
    printf("\n========== O matrix comparison (full FA2 mode) ==========\n");
    compare("O (FA2)", h_O_gpu.data(), h_O_ref.data(),
            N, d, qkv_elems, 0.02f);
